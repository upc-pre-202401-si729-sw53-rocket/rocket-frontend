    ...
    <div>
      ...
      <DataTable :value="classrooms" tableStyle="min-width: 50rem">
        ...
        <Column header="Actions">
          <template #body="slotProps">
            <div>
              <Button label="Edit" @click="editClassroom(slotProps.data)" />
            </div>
          </template>
        </Column>
...
export default {
  ...
  methods: {
    fetchData() {
      apiService.getClassrooms()
        .then(response => {
          ...
          console.log('State of classrooms:', this.classrooms);
        })
        .catch(error => {
    ...
    addNewClassroom() {
      // Aquí debes agregar el código para enviar el nuevo classroom al backend
      apiService.addClassroom(this.newClassroom)
        .then(() => {
          this.fetchData();
          this.showForm = false;
          this.newClassroom = {
    ...
    deleteClassroom(data) {
      ...
      apiService.deleteClassroom(data.id)
        .then(() => {
          this.fetchData();
        })
        .catch(error => {
